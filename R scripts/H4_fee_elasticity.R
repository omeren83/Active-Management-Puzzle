...
build_h4_table <- function(samp, fe_string,
  ...
  row_specs <- list(
    ...
    list(parts = "ExpRatio", label = "Expense ratio ($\\theta$)"),
    list(parts = "EXP_STATE",
         label = "Exp.\\ ratio $\\times$ Sent.\\ ($\\delta_F$)"),
    list(parts = "R_LOW_EXP_STATE",
         label = "$R^{LOW}\\times$ Exp.\\ $\\times$ Sent.\\ ($\\delta^F_1$)"),
    list(parts = "R_MID_EXP_STATE",
         label = "$R^{MID}\\times$ Exp.\\ $\\times$ Sent.\\ ($\\delta^F_2$)"),
    list(parts = "R_HIGH_EXP_STATE",
         label = "$R^{HIGH}\\times$ Exp.\\ $\\times$ Sent.\\ ($\\delta^F_3$)"),
  ...
  joint_row <- c(paste0("Joint $\\delta_F=\\delta^F_q=0$ ",
                        "($\\chi^2_4$, $p$)"),
                 "--",
  ...
  delta_row <- c("Headline $\\delta_F>0$ ($z$, 1-sided $p$)",
                 "--",
