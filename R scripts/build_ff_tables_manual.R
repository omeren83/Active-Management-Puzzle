...
t7_tex <- c(
  ...
  "\\footnotesize",
  paste0(
    "\\item Annualised \\citet{Carhart1997} four-factor alpha (\\%) from ",
    "regressing the monthly aggregate portfolio return of each group on the ",
    "market, size, value, and momentum factors, following \\citet{FamaFrench2010}. ",
    ...
    "\\citet{FamaFrench2010} sample (Jan 1984--Sep 2006), enabling ",
...
t9_tex <- c(
  ...
  "\\footnotesize",
  paste0(
    "\\item Bootstrap procedure follows \\citet{FamaFrench2010}. ",
    ...
    "dependence. The \\citet{Carhart1997} four-factor model is re-estimated on ",
    ...
    "lag are used throughout. Compare with \\citet{FamaFrench2010}, Table~III."
  ),
  "\\end{tablenotes}",
  "\\end{threeparttable}",
  "\\end{table}"
...
t10_tex <- c(
  ...
  paste0(
    ...
    "estimated following \\citet{Storey2002} and \\citet{BarrasScailletWermers2010}, ",
    "using p-values from Newey--West ",
    "$t$-tests on full-period \\citet{Carhart1997} four-factor alphas. The ",
    "estimator is $\\hat{\\pi}_0 = |\\{p_i > \\lambda\\}| \\,/\\, [N(1-\\lambda)]$, ",
    "where $\\lambda = 0.5$ is the standard tuning parameter following ",
    "\\citet{Storey2002}. Funds with p-values exceeding $\\lambda$ are unlikely to ",
    ...
    "Compare with \\citet{FamaFrench2010}, ",
...
t11_tex <- c(
  ...
  "\\footnotesize",
  paste0(
    "\\item Decomposition follows \\citet{BarrasScailletWermers2010}, ",
...
t13_tex <- c(
  ...
  paste0(
    ...
    ". Compare with \\citet{FamaFrench2010}, Table~I."
  ),
  "\\end{tablenotes}",
  "\\end{threeparttable}",
  "\\end{table}"
